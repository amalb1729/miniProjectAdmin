...
import { myContext } from '../../App'; // or wherever your context is
...
function ImageUploader({ item }) {
    ...
    const onSuccess = async (res, token = accessToken) => {
        try {
            if (!token) {
                token = await refreshRequest();
            }
            let response = await fetch(`/api/item/upload/${item._id}`, {
                method: "POST",
                headers: {
            ...
            if (response.status === 401) {
                token = await refreshRequest();
                response = await fetch(`/api/item/upload/${item._id}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${token}`
