...
function EditAnnouncementModal({ announcement, onClose, onUpdate, accessToken, refreshRequest }) {
  ...
  useEffect(() => {
    if (announcement) {
      setFormData({
        ...
        priority: announcement.priority !== undefined ? announcement.priority : 0
      });
    }
  }, [announcement]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: err.message || "Failed to update announcement. Please try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={true} closeModal={onClose}>
      <div className="add-modal-content">
        <h2 className="add-modal-title">Edit Announcement</h2>
